...
public class GatewayMQTTSN {
    ...
    private HashMap<MQTTSNPacket, Integer> waitingQoSMap = new HashMap<>(); // Pesan PUBLISH key, WirelessNodeId value
    
    private HashMap<Integer, Integer> globalIdMap = new HashMap<>(); // WirelessNodeId key, Global ID value
                ...
                if (nodeSensorMap.containsValue(wirelessNodeId)){
                    ...
                    if (topicName == null){ // Ga ketemu mappingnya, jadi harus send PUBACK ke sensor
                        ...
                        if (messageId != 0){
                            ...
                            int qos = (flags & 0x60) >> 5; // Ambil bit QoS
                            if (qos == 1 || qos == 2) {
                ...
                try {
                    ...
                    MqttActionListener pubcompListener = new MqttActionListener() {
                        ...
                        public void onSuccess(IMqttToken asyncActionToken) {
                            ...
                            encapsulateAndSendToGW(wirelessNodeId, pubcompPacket);
        ...
        try {
            MqttActionListener pubcompListener = new MqttActionListener() {
                ...
                public void onSuccess(IMqttToken asyncActionToken) {
                    ...
                    MQTTSNPacket pubcompPacket = new MQTTSNPacket();
                    pubcompPacket.setPUBCOMP(10);
                    // encapsulateAndSendToGW(waitingQoSMap.get(mqttsnPacket), pubcompPacket);
                    System.out.println("PUBCOMP send  to Node Sensor: "+10);
                }
                        ...
                        if (qosBits == 1){
                            ...
                            MqttActionListener pubListener = new MqttActionListener() {
                                ...
                                public void onSuccess(IMqttToken asyncActionToken) {
                                    ...
                                    encapsulateAndSendToGW(waitingQoSMap.remove(mqttsnPacket), pubackPacket);
                                }
                            ...
                            MqttActionListener pubrecListener = new MqttActionListener() {
                                ...
                                public void onSuccess(IMqttToken asyncActionToken) {
                                    ...
                                    encapsulateAndSendToGW(waitingQoSMap.remove(mqttsnPacket), pubrecPacket);
