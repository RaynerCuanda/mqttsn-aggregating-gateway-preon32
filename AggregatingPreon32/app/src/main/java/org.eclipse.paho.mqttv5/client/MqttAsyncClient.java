...
public class MqttAsyncClient implements MqttClientInterface, IMqttAsyncClient {
	...
	public IMqttToken sendPubRel(int msgId, MqttActionListener callback) throws MqttException{
		final String methodName = "sendPubRel";

		MqttToken token = new MqttToken(getClientId());
		MqttPubRel rel = new MqttPubRel(MqttReturnCode.RETURN_CODE_SUCCESS, msgId,
					new MqttProperties());
		token.internalTok.setMessageID(msgId);
		token.setActionCallback(callback);
